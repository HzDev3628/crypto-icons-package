import type { SVGProps } from 'react'

export const BeamIcon = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg
      width="133"
      height="133"
      viewBox="0 0 133 133"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g clipPath="url(#clip0_2019_2740)">
        <mask
          id="mask0_2019_2740"
          maskUnits="userSpaceOnUse"
          x="0"
          y="0"
          width="133"
          height="133"
        >
          <path d="M132.5 0.5H0.5V132.5H132.5V0.5Z" fill="white" />
        </mask>
        <g mask="url(#mask0_2019_2740)">
          <mask
            id="mask1_2019_2740"
            maskUnits="userSpaceOnUse"
            x="0"
            y="0"
            width="133"
            height="133"
          >
            <path
              d="M132.5 66.5C132.5 30.0492 102.951 0.5 66.5 0.5C30.0492 0.5 0.5 30.0492 0.5 66.5C0.5 102.951 30.0492 132.5 66.5 132.5C102.951 132.5 132.5 102.951 132.5 66.5Z"
              fill="white"
            />
          </mask>
          <g mask="url(#mask1_2019_2740)">
            <path
              d="M132.5 66.5C132.5 30.0492 102.951 0.5 66.5 0.5C30.0492 0.5 0.5 30.0492 0.5 66.5C0.5 102.951 30.0492 132.5 66.5 132.5C102.951 132.5 132.5 102.951 132.5 66.5Z"
              fill="white"
            />
            <path
              d="M-172.75 66.5H338.75V99.5H-161.012L-172.75 66.5Z"
              fill="url(#paint0_linear_2019_2740)"
            />
            <path
              d="M-205.75 0.5H338.75V33.5H-192.629L-205.75 0.5Z"
              fill="url(#paint1_linear_2019_2740)"
            />
            <path
              d="M-160.375 99.5H338.75V132.5H-160.375V99.5Z"
              fill="url(#paint2_linear_2019_2740)"
            />
            <path
              d="M-185.125 33.5H338.75V66.5H-185.125V33.5Z"
              fill="url(#paint3_linear_2019_2740)"
            />
            <path
              d="M-185.125 33.5H338.75V66.5H-185.125V33.5Z"
              fill="url(#paint4_linear_2019_2740)"
            />
          </g>
        </g>
      </g>
      <defs>
        <linearGradient
          id="paint0_linear_2019_2740"
          x1="68.098"
          y1="83.825"
          x2="68.098"
          y2="120.641"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#F1E869" />
          <stop offset="0.328125" stopColor="#FEA514" />
          <stop offset="0.598958" stopColor="#FF4539" />
        </linearGradient>
        <linearGradient
          id="paint1_linear_2019_2740"
          x1="68.4561"
          y1="17.825"
          x2="68.4536"
          y2="54.6406"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#BBDBFF" />
          <stop offset="0.328125" stopColor="#139EDD" />
          <stop offset="0.598958" stopColor="#B9F7EA" />
        </linearGradient>
        <linearGradient
          id="paint2_linear_2019_2740"
          x1="67.7276"
          y1="116.825"
          x2="67.7247"
          y2="153.641"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#BBFFCA" />
          <stop offset="0.328125" stopColor="#48DD13" />
          <stop offset="0.598958" stopColor="#008805" />
        </linearGradient>
        <linearGradient
          id="paint3_linear_2019_2740"
          x1="66.7318"
          y1="37.5741"
          x2="66.7318"
          y2="67.8241"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#FF6A9A" />
          <stop offset="0.529819" stopColor="#FF5544" />
          <stop offset="0.955545" stopColor="#E63E33" />
        </linearGradient>
        <linearGradient
          id="paint4_linear_2019_2740"
          x1="61.5512"
          y1="50.825"
          x2="61.5484"
          y2="87.6406"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#FF6B6B" />
          <stop offset="0.328125" stopColor="#FE1414" />
          <stop offset="0.598958" stopColor="#8E0900" />
        </linearGradient>
        <clipPath id="clip0_2019_2740">
          <rect x="0.5" y="0.5" width="132" height="132" rx="66" fill="white" />
        </clipPath>
      </defs>
    </svg>
  )
}
